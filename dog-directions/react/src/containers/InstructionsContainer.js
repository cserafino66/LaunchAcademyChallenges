...
class InstructionsContainer
 extends Component {
  constructor(props) {
    ...
    this.chosenStepSelector = this.chosenStepSelector.bind(this)
  }

  chosenStepSelector(id) {
    this.setState({chosenStep: id})
  }
  render(){
    ...
    let steps = directions.map(direction => {
      let className;
      if (direction.id === this.state.chosenStep) {
        className = 'selected';
      } else {
        className = null;
      }
      return(
        <StepTile
          ...
          key={direction.id}
          id={direction.id}
          setSelectedStep={this.chosenStepSelector}
          className={className}
        />
      )
    })
